...
## Acceptance
- [ ] Sage's `app/llm/client.py` is a verbatim port of bread's `src/llm/client.py` (same ChatAttempt fields, OllamaClientConfig parity, _build_options, _build_messages, _run_attempt/_stream_chat/_single_chat, status_snapshot, _response_to_chunk). Settings mapping only.
...
- app/llm/client.py now bread-identical OllamaClient (ChatAttempt 5 features, multi-backend, _build_options/_response_to_chunk, degradation ladder) + Sage inflight + _strip_leaked_calls in _response_to_chunk + compat stream_chat (as_events strings vs dicts)
...
- app/llm/client.py now bread-identical SageOllamaClient (ChatAttempt 5 features, _build_options/_build_messages/_response_to_chunk, multi-backend, degradation ladder, status_snapshot) + Sage inflight + _strip_leaked_calls in _response_to_chunk + compat stream_chat (as_events strings vs dicts) + get_llm_client/reset_llm_client
