...
def test_id_format():
    units = [ExtractedUnit("abc " * 30)]
    chunks = chunk_units(units, file_id="f1a2", chunk_chars=50, overlap=10)
    assert chunks[0].id == "f1a2:0:0"
    assert chunks[1].id == "f1a2:0:1"
    assert all(c.id.startswith("f1a2:") for c in chunks)
