...
PHASE_PLAYBOOK = (
    ...
    "idea per turn, then key step: end the turn by calling "
    "record_step_actions with a single 'continue' action (label like \"Continue\" "
    "or \"Next\") so the learner gets a button to move on when they've finished "
    "reading. That Continue prompt should advance to the next idea. When a node is "
...
SLIM_PHASE_PLAYBOOK = (
    ...
    "learner picked (A=0, B=1, C=2). Example: '1: B' means the first question "
    "answered with option B, so call grade_answer with that question's id and "
    "choice_index 1. Do not reveal the correct answer before grading. After "
    "all answers are graded, summarize and call build_plan once. "
    ...
    "label 'Continue', prompt 'advance to the next idea') so the learner gets "
    "a button to move on. Call advance_lesson only when the learner asks to "
    "continue. Do not quiz mid-lesson; run_final_quiz comes at the end. "
